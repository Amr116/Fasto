  ...
  = case exp of
    ...
    | In.And (e1, e2, pos)
      => let val (t1, e1') = checkExp ftab vtab e1
             val (t2, e2') = checkExp ftab vtab e2
         in case (t1, t2 ) of
             (Bool, Bool ) => (Bool, Out.And (e1', e2', pos))
           | (Bool, _ ) => raise Error ("Second Argument of && "^ ppType t2 ^"is not a Bool",pos)
           | (_, Bool ) => raise Error ("First Argument of && "^ ppType t1 ^" is not a Bool",pos)
           | (_, _)     => raise Error ("Cannot Bitwise "^ ppType t1 ^
                                             "and "^ppType t2^"for && bitwise operation need boolean", pos) 
         end
    ...
    | In.Or (e1, e2, pos)
      => let val (t1, e1') = checkExp ftab vtab e1
             val (t2, e2') = checkExp ftab vtab e2
         in case (t1, t2 ) of
              (Bool, Bool ) => (Bool, Out.Or (e1', e2', pos))
           | (Bool, _ ) => raise Error ("First Argument of || "^ ppType t1 ^"is not a Bool",pos)
           | (_, Bool ) => raise Error ("Second Argument of || "^ ppType t2 ^" is not a Bool",pos) 
           | (_, _) => raise Error ("Cannot compare "^ ppType t1 ^
                                             "and "^ppType t2^"for || bitwise operation  need boolean", pos) 
         end
    ...
    | In.Not (e, pos)
      => let val (t, e') = checkExp ftab vtab e
         in case t of
           ...
           | (_)    => raise Error ("Boolean expected "^ppType t^" to",pos)
         end
    ...
    | In.Negate (e, pos)
      => let val (t, e') = checkExp ftab vtab e
         in case t of
           ...
           | (_)   => raise Error ("Int expected "^ppType t^" to",pos)
         end
      ...
      => let val (n_type, n_dec) = checkExp ftab vtab n_exp
         ...
         in if elem_type = f_arg_type
            then if elem_type = n_type
                 ...
                 else raise (err ("neutral element and element type do not match", n_type))
            else raise err ("array element type and function argument type do not match", elem_type)
         end
               ...
               case checkFunArg (f, vtab, ftab, pos) of
                 ...
                 | (_,  res, args) =>
                   raise Error ("Filter: incompatible function type of "
                                ^ In.ppFunArg 0 f ^ ":" ^ showFunType (args, res), pos)
         in if f_res_type = Bool andalso elem_type = f_arg_type
            then (Array elem_type,
                  Out.Filter (f', arr_exp_dec, elem_type, pos))
            else raise Error ("Filter: type of array elements and function argument do not match", pos)
         end
...
and checkFunArg (In.FunName fname, vtab, ftab, pos) =
    ...
    (*          start          *)
    (***************************)
| checkFunArg (In.Lambda( rtp,params,exp,pos), vtab, ftab, pos1) =
    let val lambdaFun = In.FunDec("f",rtp,params,exp, pos) 
        val lambda' = checkFunWithVtable(lambdaFun ,vtab, ftab,pos)
        val (Out.FunDec(x, tp, param, bodyPrime, pos2)) = lambda'
        val argTypes = map(fn (Param(x, tp)) => tp) params
    in
     (Out.Lambda(rtp, params, bodyPrime, pos), rtp, argTypes)
    end
    (***************************)
    (*      Task 3  Lambda     *)
    (*          end            *)
