...
(*************************************************)
    ...
    (***********************************)
    (* Project solution: Taks 1        *)
    (*             start               *)
    (***********************************)

    | Times (e1, e2, pos) =>
       let val t1 = newName"mul_L"
           val t2 = newName"mul_R"
           val code1 = compileExp e1 vtable t1
           val code2 = compileExp e2 vtable t2
       in
    ...
    | And (e1, e2, pos) =>
         let val t1 = newName "and_L"
            ...
            val falsk = newName "false_compation"
            val end_And = newName "end_And"
            val code1 = compileExp e1 vtable t1
            val code2 = compileExp e2 vtable t2
            val footer = [Mips.LI(place, "1"),
                          Mips.J(end_And),
                          Mips.LABEL falsk,
                          Mips.MOVE(place, "0"),
                          Mips.LABEL end_And]
    ...
    | Or (e1, e2, pos) =>
         let val t1    = newName "Or_L"
             val t2    = newName "Or_R"
             val sandt = newName "true_comparation"
             val end_or= newName "end_Or"
             val code1 = compileExp e1 vtable t1
             val code2 = compileExp e2 vtable t2
             val footer= [Mips.LI(place, "0"),
                          Mips.J(end_or),
                          Mips.LABEL sandt,
                          Mips.LI(place, "1"),
                          Mips.LABEL end_or]
...
(*
    | Great (e1, e2, pos) =>
        let val t1 = newName"lt_L"
            ...
            val code2 = compileExp e2 vtable t2
        in  code1 @ code2 @
            [Mips.SUB (place,t2,t1), MIPS.SLT(place,place,"0") ]
        end
*)
   ...
   (***********************************)
    | Scan(binop, acc_exp, arr_exp, ret_type, pos) =>
        ...
        let val acc_reg      = newName "acc_reg"     (*Accumulator register *)
            val old_arr_reg  = newName "old_arr_reg" (*Old array address    *)
            val new_arr_reg  = newName "new_arr_reg" (*New array address    *)
            val old_elem_reg = newName "old_elem_reg"(*Old element register *)
            val new_elem_reg = newName "new_elem_reg"(*New element register *)
            val arr_size_reg = newName "arr_size_reg"(*New array size = old array size + 1 *)
            val i_reg        = newName "i_reg"       (*Point Reister        *)
            val tmp_reg      = newName "tmp_reg"     (*Temp register        *)
            val loop_beg      = newName "loop_beg"   (*Label Start          *)
            val loop_end      = newName "loop_end"   (*Label end            *)
             ...
             * - Point old_elem_reg to first elem in input array
            ...
            (* Dynalloc call to Allocate space *)
            val allocate_space = dynalloc(tmp_reg, place, ret_type)
            (* place -> newarr[0] 
             * set new array = [size,-> acc_exp <-, ]
             * *)
            val header_1 = [Mips.ADDI(new_elem_reg, place, "4"),
                            Mips.SW(acc_reg, new_elem_reg, "0")]
            val while_header = [Mips.LABEL loop_beg,
                                Mips.SUB (tmp_reg, i_reg, arr_size_reg),
                                Mips.BGEZ (tmp_reg, loop_end) ]
            (* While Loop:
             * Increment new array pointer
             * Load old_elem
             *)
            val while_load =
              case getElemSize ret_type of
            ...
            (* Call f(acc, old_array_elem) -> acc *)
            val while_fun = applyFunArg(binop, [acc_reg, tmp_reg], vtable, acc_reg, pos)
           
           (* Save acc to new array pointer *)
            val while_save =
              case getElemSize ret_type of
           ...
           (* increment old_elem pointer and i and jump *)
            val footer =  [ Mips.ADDI (i_reg, i_reg, "1"),
                            Mips.J loop_beg,
                            Mips.LABEL loop_end]
         in acc_code
            @ arr_code
    ...
    | Filter (f, arr_exp, tp, pos) =>
       let val new_array_len_reg = newName "new_array_len_reg"(* new array size *)
           val old_array_len_reg = newName "old_array_len_reg"(* old array size *)
           val old_array_reg     = newName "old_array_reg"    (* address of old array *)
           val old_elem_reg      = newName "old_elem_reg"     (* address of old single element *)
           val new_elem_reg      = newName "new_elem_reg"     (* address of new single element *)
           val i_reg             = newName "i_reg"            (* point counter *)
           val tmp1_reg           = newName "tmp1_reg"        (* temporary to each element *)
           val tmp2_reg          = newName "tmp2_reg"         (* check true eller false*) 
           val loop_beg          = newName "loop_beg"         (* Label begin*)
           val loop_end          = newName "loop_end"         (* Label end  *)
           ...
           (* Dynalloc call to Allocate space *)
           val allocate_space = dynalloc(old_array_len_reg, place, tp)
           (* Ready elem_regs for loop *)
           val while_header = [Mips.ADDI(old_elem_reg, old_array_reg, "4"),
                               Mips.ADDI(new_elem_reg, place, "4"),
                               Mips.LABEL(loop_beg),
                               Mips.SUB(tmp1_reg, i_reg, old_array_len_reg),
                               Mips.BGEZ(tmp1_reg, loop_end)]
           (* Get the next elem of the old array and increment the pointer *)
           val while_load = 
           case getElemSize tp of
                One => [Mips.LB(tmp1_reg, old_elem_reg, "0"),
                        Mips.ADDI(old_elem_reg, old_elem_reg, "1")]
             | Four => [Mips.LW(tmp1_reg, old_elem_reg, "0"),
                        Mips.ADDI(old_elem_reg, old_elem_reg, "4")]
           val while_fun = applyFunArg(f, [tmp1_reg], vtable, tmp2_reg, pos)
           val while_if =
           case getElemSize tp of
                One => [Mips.BEQ(tmp2_reg, "0", if_f),
                        Mips.SB(tmp1_reg, new_elem_reg, "0"),
                        Mips.ADDI(new_elem_reg, new_elem_reg, "1"),
                        Mips.ADDI(new_array_len_reg, new_array_len_reg, "1")]
             | Four => [Mips.BEQ(tmp2_reg, "0", if_f),
                        Mips.SW(tmp1_reg, new_elem_reg, "0"),
...
(****************)
    ...
    | applyFunArg (Lambda ( rettype, params, exp, lampos), args, vtab, place, pos) =
      let fun bindArg (Param(name, tp), arg, vtab) = SymTab.bind name arg vtab
          val empty_vtab' = SymTab.empty()
          val vtab'       = ListPair.foldr bindArg empty_vtab' (params, args)
      in 
          compileExp exp vtab' place
      end
(*
