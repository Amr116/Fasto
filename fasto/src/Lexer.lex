...
(* This one is language specific, yet very common. Alternative would
 ...
 fun keyword (s, pos) =
     case s of
       ...
       | "fun"          => Parser.FUN pos
       | "not"          => Parser.NOT pos    (* boolean − negation unary operator*)
       | "true"         => Parser.TRUE pos   (* boolean − true value *)
       | "false"        => Parser.FALSE pos  (* boolean − false value *)
...
rule Token = parse
  ...
  | `*`                 { Parser.TIMES  (getPos lexbuf) } (* integer − multiplication operator *)
  | `/` 		{ Parser.DIVIDE (getPos lexbuf) } (* integer − division operator *)
  | `~`                 { Parser.NEG    (getPos lexbuf) } (* integer − negation unary operator *)
  | "&&"                { Parser.AND    (getPos lexbuf) } (* boolean − and operator *)
  | "||"                { Parser.OR    (getPos lexbuf) }  (* boolean − or operator *)		
