...
fun evalExp ( Constant (v,_), vtab, ftab ) = v
 ...
 | evalExp ( And(e1, e2, pos), vtab, ftab ) =
        ...
        in  case res1 of
          ...
          | _             => raise Error ("And Operation must have bool value", pos)
        end
 ...
 | evalExp ( Or(e1, e2, pos), vtab, ftab ) =
        ...
        in  case res1 of
          ...
          | _             => raise Error ("Or operation must have bool value", pos)
        end
 ...
 | evalExp ( Negate(e1, pos), vtab, ftab ) =
        ...
        in case res1 of
              IntVal n => IntVal (~n)
            | _        => raise Error ("The experation in Negate operation has to be Int" , pos)
        end
 ...
 | evalExp ( Not(e1, pos), vtab, ftab ) =
        ...
        in case res1 of 
            ...
            | _        => raise Error ("The experation in Not operation has to be boolean", pos) 
             end
...
(* 
 | evalExp ( Great(e1, e2, pos), vtab, ftab ) =
        let val r1 = evalExp(e1, vtab, ftab)
            val r2 = evalExp(e2, vtab, ftab)
        in  evalRelop(op >, r1, r2, pos)   (* <? *)
        end
...
and evalFunArg (FunName fid, vtab, ftab, callpos) =
 ...
 |   evalFunArg (Lambda (rtp,params,body,pos),vtab, ftab, callpos) =
      let
        val dummy = FunDec ("dummy", rtp, params, body, pos) 
        in
          (fn aargs => callFunWithVtable(dummy, aargs, vtab, ftab, callpos), rtp)
      end
